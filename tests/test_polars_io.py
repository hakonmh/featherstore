...
@pytest.mark.parametrize(
    ...
    [
        ...
        (make_table(sorted_datetime_index, astype="pandas"), ["2021-01-07", "2021-01-20"]),
...
def test_filtering_rows_before_low_with_string_index(
    ...
    partition_size = get_partition_size(original_df, basic_data["num_partitions"])
    store.write_table(
        basic_data["table_name"], original_df, index='index', partition_size=partition_size
    )
    # Act
    df = store.read_polars(basic_data["table_name"], rows=ROWS)
...
def test_filtering_rows_after_low_with_datetime_index(
    ...
    partition_size = get_partition_size(original_df, basic_data["num_partitions"])
    store.write_table(
        basic_data["table_name"], original_df, index='Date', partition_size=partition_size
    )
    # Act
    df = store.read_polars(basic_data["table_name"], rows=ROWS)
